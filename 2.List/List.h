...
public:
	...
	using ValueType = typename List::ValueType; // 위에 template에서 typename List 전체를 가져옴
...
public:
	...
	void Add(T&& value)
	{
		...
		{
			// 크기 재할당 (2배 크기로 재할당). 
			// -> 배열의 크기 처음에 고정하는걸 크기 재할당으로 해결함
			// 동적 배열을 쓸때는 재할당을 덜하게 하는게 좋다 -> 자주 new/delete가 발생하면 성능이슈...
			Reallocate(capacity * 2);
		}
