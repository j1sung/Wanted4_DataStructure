...
// key를 정수로 바꿔서 버킷 인덱스를 계산하는데 쓰는 함수.
...
static int GenerateHash(const std::string& keyString)
{
    ...
    "ABCD" [A, B, C, D] = [65, 66, 67, 68]
     "ABCD" == "DCBA" 이게 성립한다는 것은
     다른 key에서 해시값이 같아서 해시 충돌인 것이다.
     이렇게 해시 충돌이 나면 좋은 해시함수가 아니다.
     단순히 문자 값을 다 더하는 방식이면
    "ABCD"와 "DCBA"는 같은 합이 나와서 해시 충돌이 쉽게 난다.
    그래서 “순서까지 반영되는” 해시가 더 좋다.
    */
    ...
    // 생성한 해시 값 반환.
    // 오버플로우로 인한 음수 변환 가능성이 있기 때문에 절대값으로 변환.
    return std::abs(hash);
}
// 버킷카운트를 소수로 했다.
// 해시 충돌을 막는 해시 함수를 만든다.
// static으로 해시 함수를 다뤄서 private으로 한다.
/*
보통 해시테이블을 직접 구현해서 쓰지 않는다.
그래서 해시테이블이 멤버함수로 가질필요가 없는디
이건 직접 구현해서 static으로 private성질을 줬다?
*/
