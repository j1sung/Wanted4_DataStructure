...
public:
	...
	bool Enqueue(const T& value)
	{
		...
		rear = (rear + 1) % (capacity + 1); // 모듈러 - 원형큐 구현
		data[rear] = value;
		return true;
	...
	bool Dequeue(T& outValue) 
	{
		...
		// 값 비우기 (초기화)
		data[front] = T();
		//memset(&data[front], 0, sizeof(T)); -> 이렇게 초기화도 가능
		return true;
	}
	...
	// 출력 함수.
	// 출력할때 dequeue하면 삭제까지 되어서 삭제 안하려고 인덱스 계산이 까다로워 짐
	void Print()
	{
		std::cout << "큐 내용 출력: ";
	...
	inline bool IsFull() const { return ((rear + 1)%(capacity + 1)) == front; } // % 모듈러로 나누면 자기 크기에서 0으로 돌아감.
